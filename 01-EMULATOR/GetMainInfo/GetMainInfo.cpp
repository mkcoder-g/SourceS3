...
int _tmain(int argc, _TCHAR* argv[]) // OK
{
	...
	{
		((BYTE*)&info)[n] ^= (BYTE)(0xBA^LOBYTE(n));
		((BYTE*)&info)[n] -= (BYTE)(0x2D^HIBYTE(n));
		((BYTE*)&info)[n] += (BYTE)(0xA6^HIBYTE(n));
	}

	HANDLE file = CreateFile(".\\Path\\load.xmu",GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_ARCHIVE,0);
