...
case COMMAND_PK_CLEAR:
		...
		case COMMAND_CUSTOM_PICK:
			gCustomPick.CommandCustomPick(lpObj);
			break;
		case COMMAND_CUSTOM_PICK_SET:
			gCustomPick.CommandCustomPickSet(lpObj,argument);
			break;
		case COMMAND_CUSTOM_PICK_CLEAR:
			gCustomPick.CommandCustomPickClear(lpObj);
			break;
...
void CCommandManager::DiscountRequirement(LPOBJ lpObj,int index) // OK
{
	...
	if(lpInfo.Coin1[lpObj->AccountLevel] > 0)
	{
		gObjCoinSub(lpObj->Index,lpInfo.Coin1[lpObj->AccountLevel]);
	}
...
void CCommandManager::CommandSetCoin(LPOBJ lpObj,char* arg) // OK
{
	...
	if(Coin1 > 0)
	{
		gObjCoinAdd(lpTarget->Index,Coin1);
		gLog.Output(LOG_COMMAND,"[CommandGMSetCoin][%s][%s] - [%s][%s] Increase Coin1[%d]",lpObj->Account,lpObj->Name,lpTarget->Account,lpTarget->Name,Coin1);
	}

	if(Coin1 < 0)
	{
		gObjCoinSub(lpTarget->Index,Coin1);
		gLog.Output(LOG_COMMAND,"[CommandSetCoin][%s][%s] - [%s][%s] Decrease Coin1[%d]",lpObj->Account,lpObj->Name,lpTarget->Account,lpTarget->Name,abs(Coin1));
	}
...
void CCommandManager::DGCommandResetRecv(SDHP_COMMAND_RESET_RECV* lpMsg) // OK
{
	...
	gObjCoinSub(lpObj->Index,gResetTable.GetResetCoin1(lpObj));
...
void CCommandManager::DGCommandMasterResetRecv(SDHP_COMMAND_MASTER_RESET_RECV* lpMsg) // OK
{
	...
	gObjCoinSub(lpObj->Index,gServerInfo.m_CommandMasterResetCoin1[lpObj->AccountLevel]);
