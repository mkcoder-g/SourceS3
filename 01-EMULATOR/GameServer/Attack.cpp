...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(damage > 0)
	{
		...
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			if(lpTarget->Inventory[INVENTORY_SLOT_HELPER].IsItem() == 0 || (lpTarget->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,2) && gServerInfo.m_DamageStuckOnPetUniria != 0) || (lpTarget->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,3) && gServerInfo.m_DamageStuckOnPetDinorant != 0) || (lpTarget->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,4) && gServerInfo.m_DamageStuckOnPetDarkHorse != 0) || (lpTarget->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,37) && gServerInfo.m_DamageStuckOnPetFenrir != 0))
			{
