...
void CItemManager::GetItemBmd(char* path) // OK
{
	...
	{
		LogAdd(LOG_RED,"[%s] Could not alloc memory for write file.",path);
		return;
	}

	if(WriteFile(file,&info,sizeof(info),0,0) == 0)
	{
		LogAdd(LOG_RED,"[%s] Could not write file.",path);
		CloseHandle(file);
		return;
	}
	...
	if(WriteFile(file,&Checksum,sizeof(Checksum),0,0) == 0)
	{
		LogAdd(LOG_RED,"[%s] Could not write checksum.",path);
		CloseHandle(file);
		return;
	}
...
bool CItemManager::CheckItemMoveToChaos(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	/*if (TargetFlag != 8 && TargetFlag != 16 && lpItem->IsLuckyItem() != 0) // CHECK
	{
		return 0;
	}*/
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) // Dark Horse, Dark Reaven
	{
		...
		if(type == 1)
		{
			gObjCoinSub(aIndex,item.m_CoinValue);
		}
		else
		{
