...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,gMessage.GetMessage(CustomMonsterInfo.RewardMessage),CustomMonsterInfo.RewardCoin1);
	}

	if(CustomMonsterInfo.RewardCoin1 > 0)
	{
		gObjCoinAdd(lpTarget->Index,CustomMonsterInfo.RewardCoin1);
	}
}
