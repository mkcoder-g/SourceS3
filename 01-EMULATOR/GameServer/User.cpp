	...
	for(int n = 0; n < MAX_OBJECT; n++)
	{
		if(gObj[n].Connected != OBJECT_OFFLINE)
		{
			if(gObj[n].Type == OBJECT_USER)
			{
				...
				if(gObj[n].Attack.Offline != 0 || gObj[n].PShopCustomOffline != 0)
				{
					TotalOffline++;
				}
			}
...
void gObjCoinAdd(int aIndex,int Coin1) // OK
{
	gMuCashShop.GDMuCashPointAddPointSaveSend(aIndex,Coin1);
}

void gObjCoinSub(int aIndex,int Coin1) // OK
{
...
void gObjUserKill(int aIndex) // OK
{
	...
	if(lpObj->Attack.Offline != 0 || lpObj->PShopCustomOffline != 0)
	{
		CloseClient(aIndex);
