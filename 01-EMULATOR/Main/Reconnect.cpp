...
void InitReconnect() // OK
{
	...
	SetCompleteHook(0xE9,0x006DD7F3,&ReconnectGetAccountInfo);

	SetCompleteHook(0xE9,0x006D0C36,&ReconnectCheckConnection);

	SetCompleteHook(0xE9,0x0050FD92,&ReconnectCloseSocket);

	SetCompleteHook(0xE9,0x006BAFE6,&ReconnectMenuExitGame);

	SetCompleteHook(0xE8,0x00414345,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x004E24A7,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x0050A61F,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x006CD138,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x006DDA19,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x005FFCEE,&ReconnectMainProc);
}
...
void ReconnectMainProc() // OK
{
	...
	if((GetTickCount()-ReconnectMaxTime) > ReconnectMaxWait)
	{
		...
		((void(__thiscall*)(void*))0x0050FD80)((void*)MAIN_ACTIVE_SOCKET);
		return;
	}
...
void ReconnecGameServerAuth() // OK
{
	...
	pMsg.ClientVersion[0] = (*(BYTE*)(0x007D1AA0))-1;

	pMsg.ClientVersion[1] = (*(BYTE*)(0x007D1AA1))-2;

	pMsg.ClientVersion[2] = (*(BYTE*)(0x007D1AA2))-3;

	pMsg.ClientVersion[3] = (*(BYTE*)(0x007D1AA3))-4;

	pMsg.ClientVersion[4] = (*(BYTE*)(0x007D1AA4))-5;

	memcpy(pMsg.ClientSerial,(void*)0x007D1AA8,sizeof(pMsg.ClientSerial));
...
void ReconnectViewportDestroy() // OK
{
	...
	do
	{
		...
		{
			if((ViewportAddress=(pViewportAddress+(count*0x484))) == 0)
			{
				continue;
			}
			...
			info.index[0] = SET_NUMBERHB((*(WORD*)(ViewportAddress+0x234)));
			info.index[1] = SET_NUMBERLB((*(WORD*)(ViewportAddress+0x234)));
...
BOOL ReconnectCreateConnection(char* address,WORD port) // OK
{
	...
	return ((BOOL(*)(char*,WORD))0x004E1C50)(address,port);
}

__declspec(naked) void ReconnectGetAccountInfo() // OK
{
	static DWORD ReconnectGetAccountInfoAddress1 = 0x006DD7FD;

	_asm
	{
		Push 0x0A
		Lea Eax,Dword Ptr Ss:[0x07903760]
		Push Eax
		...
		Add Esp,0x0C
		Push 0x0A
		Lea Edx,Dword Ptr Ss:[0x07903860]
		Push Edx
		Lea Eax,ReconnectPassword
		Push Eax
		Call [MemoryCpy]
		Add Esp,0x0C
		Mov Dword Ptr Ds:[0x0587A22C],0x01
...
{
	static DWORD ReconnectCheckConnectionAddress1 = 0x006D0C3B;
	static DWORD ReconnectCheckConnectionAddress2 = 0x006D0C76;
...
{
	static DWORD ReconnectCloseSocketAddress1 = 0x0050FD9C;
...
{
	static DWORD ReconnectMenuExitGameAddress1 = 0x006BAFEB;
