...
void DrawNewHealthBar() // OK
{
	...
	for (int n = 0; n < MAX_MAIN_VIEWPORT; n++)
	{
		if((ViewportAddress=(pViewportAddress+(n*0x484))) == 0)
		{
			continue;
		}
		...
		NEW_HEALTH_BAR* lpNewHealthBar = GetNewHealthBar(*(WORD*)(ViewportAddress+0x234),*(BYTE*)(ViewportAddress+0xBC));
		...
		if((pCursorX >= PosX) && ((float)pCursorX <= (float)PosX+LifeBarWidth) && (pCursorY >= PosY-2) && (pCursorY < PosY+6))
		{
			...
			pDrawText(PosX,PosY-7,LifeDisplay,0,0,0);
		}
