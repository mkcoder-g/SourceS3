...
bool CProtect::ReadMainFile(char* name) // OK
{
	...
	{
		((BYTE*)&this->m_MainInfo)[n] -= (BYTE)(0xA6^HIBYTE(n));
		((BYTE*)&this->m_MainInfo)[n] += (BYTE)(0x2D^HIBYTE(n));
		((BYTE*)&this->m_MainInfo)[n] ^= (BYTE)(0xBA^LOBYTE(n));
	}
...
void CProtect::CheckInstance() // OK
{
	...
	wsprintf(buff,"SSEMU_MAIN_S3TAI_%s",this->m_MainInfo.IpAddress);
