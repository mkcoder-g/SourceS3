...
void CServerDisplayer::PaintBarInfo() // OK
{
	...
	wsprintf(buff,"Usuários: %d/%d",gAccountManager.GetAccountCount(),gJoinServerMaxAccount);
	SendMessage(hWndStatusBar,SB_SETTEXT,0,(LPARAM)buff);

	wsprintf(buff,"Fila: %d/%d",gSocketManager.GetQueueSize(),MAX_MAIN_PACKET_SIZE);
